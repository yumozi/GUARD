...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, args, stride=1, downsample=None, groups=1,
        ...
        self.stride = stride
        self.per_class_bn = getattr(args, 'per_class_bn', False)
    ...
    def forward(self, x, labels=None):
        ...
        if self.per_class_bn:
            out = self.bn1(out, labels=labels)
        else:
            out = self.bn1(out)
        out = self.relu(out)
        ...
        out = self.conv2(out)
        if self.per_class_bn:
            out = self.bn2(out, labels=labels)
        else:
            out = self.bn2(out)

        if self.downsample is not None:
            if self.per_class_bn:
                identity = self.downsample(x, labels=labels)
            else:
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        out = self.conv3(out)
        if self.per_class_bn:
            out = self.bn3(out, labels=labels)
        else:
            out = self.bn3(out)
        if self.downsample is not None:
            if self.per_class_bn:
...
class ResNet(nn.Module):
    def __init__(self, block, layers, args, num_classes=1000, zero_init_residual=False,
        ...
        # so the residual branch starts with zeros, and each residual block behaves like an identity.
    ...
    def _make_layer(self, block, planes, blocks, args, stride=1, dilate=False):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, args, stride=stride, downsample=downsample, groups=self.groups,
    ...
    def forward(self, x, labels=None):
        ...
        x = self.layer1(x, labels=labels)
        x = self.layer2(x, labels=labels)
        x = self.layer3(x, labels=labels)
        x = self.layer4(x, labels=labels)
...
def resnet101(args, pretrained=False, **kwargs):
    model = ResNet(Bottleneck, [3, 4, 23, 3], args, **kwargs)
    return model