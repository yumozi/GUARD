...
from utils import str2bool, DATASETS, NUM_CLASS_MAP
...
def get_images(args, model_teacher, hook_for_display, ipc_id):
    ...
    targets_all = torch.LongTensor(np.arange(args.num_classes))

    for kk in range(0, args.num_classes, batch_size):
        targets = targets_all[kk : min(kk + batch_size, args.num_classes)].to("cuda")
            ...
            if args.cda:
                ...
                if iteration < args.milestone * iterations_per_layer:
                    if args.easy2hard_mode == "step":
                        ...
                        min_crop = lb + (1.0 - lb) * (1 - iteration / (args.milestone * iterations_per_layer))
...
def main_syn(args, ipc_id):
    ...
    model_teacher = torchvision.models.get_model(args.model, num_classes=args.num_classes)
...
def parse_args():
    ...
    """Model related flags"""
    parser.add_argument(
        "--model", type=str, default="resnet18", help="arch name from pretrained torchvision models"
    )
